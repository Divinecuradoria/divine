...
export function SupplierApplication({ embedded = false }: { embedded?: boolean }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!embedded) { window.location.replace("/painel/fornecedor"); return }
  ...
  async function submitLead(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const db = getSupabase()
      if (!db) throw new Error()
      // The database generates the ID; public submissions may only write form fields.
  ...
  return <Wrapper className={embedded ? "text-onix" : "min-h-screen bg-alabastro px-5 pb-24 pt-32 text-onix"}>{!embedded && <Header />}
    <div className="mx-auto max-w-2xl">
      <p className="text-sm uppercase tracking-widest text-bronze">Para fornecedores</p>
      <Heading className="mt-3 font-serif text-4xl">{embedded ? "Seu cadastro na Curadoria" : "Solicitar Curadoria DIVINE"}</Heading>
